OL_JAPFA.IND_UTIL.BILLING_STAGE_FFFcreate database OL_JAPFA;
...
DROP TABLE OL_JAPFA.IND.DIM_PRODUCT;
...
CREATE OR REPLACE TABLE OL_JAPFA.IND.FACT_SALES (
    ...
    FOREIGN KEY (MATERIAL) REFERENCES OL_JAPFA.IND.DIM_PRODUCT(MATERIAL),
    FOREIGN KEY (BILLING_TYPE) REFERENCES OL_JAPFA.IND.DIM_BILLING_TYPE(BILLING_TYPE)
);
...
ALTER TABLE OL_JAPFA.IND.FACT_SALES
ADD 


CREATE OR REPLACE VIEW OL_JAPFA.IND_BI.BILLING_TEAMS AS
SELECT 
    BILLING_TYPE,
    SUM(FINAL_QTY) AS TOTAL_FINAL_QTY
FROM OL_JAPFA.IND.FACT_SALES
GROUP BY BILLING_TYPE;


CREATE OR REPLACE VIEW OL_JAPFA.IND_BI.MATERIAL_TEAMS AS
SELECT 
    MATERIAL,
    SUM(FINAL_QTY) AS TOTAL_FINAL_QTY
FROM OL_JAPFA.IND.FACT_SALES
...
SELECT * FROM OL_JAPFA.IND_BI.BILLING_TEAMS;

drop view OL_JAPFA.IND_BI.MATERIAL_TEAMS;
drop view OL_JAPFA.IND_BI.BILLING_TEAMS;



select * from OL_JAPFA.IND_BI.MATERIAL_TEAMS;

-- UPDATE OL_JAPFA.IND.FACT_SALES FS
-- SET FINAL_QTY = (
--     SELECT AGG.FINAL_QTY
--     FROM OL_JAPFA.IND_SAPHANA_DEV_TN.ZSD_BAS_SALESDOC_ALL_AGG_T AGG
--     WHERE FS.BILLING_DOCUMENT = AGG.BILLING_DOCUMENT
--       AND FS.ITEM = AGG.ITEM
-- );
...
CREATE OR REPLACE PROCEDURE OL_JAPFA.IND_SAPHANA_DEV_TN.SP_LOAD_DIM_FACT_TABLES()
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
...
INSERT INTO OL_JAPFA.IND.FACT_SALES (
    ...
    MATERIAL, BILLING_TYPE, ASSIGNMENT, FINAL_QTY
)
SELECT 
    STAGE.BILLING_DOCUMENT,
    STAGE.ITEM,
    STAGE.BILLING_DATE,
    STAGE.BILLING_QTY_IN_SKU,
    STAGE.MATERIAL,
    STAGE.BILLING_TYPE,
    STAGE.ASSIGNMENT,
    AGG.FINAL_QTY
FROM OL_JAPFA.IND_UTIL.ZSD_BAS_SALESDOC_ALL_STAGE_T STAGE
LEFT JOIN OL_JAPFA.IND_SAPHANA_DEV_TN.ZSD_BAS_SALESDOC_ALL_AGG_T AGG
    ON STAGE.BILLING_DOCUMENT = AGG.BILLING_DOCUMENT
   AND STAGE.ITEM = AGG.ITEM
WHERE STAGE.BILLING_DOCUMENT IS NOT NULL
...
DROP PROCEDURE OL_JAPFA.IND_SAPHANA_DEV_TN.SP_LOAD_DIM_FACT_TABLES();

------------------------------------------

CREATE OR REPLACE PROCEDURE OL_JAPFA.IND_SAPHANA_DEV_TN.CREATE_TEAM_VIEWS()
RETURNS STRING
LANGUAGE SQL
AS
$$
BEGIN
    -- Create BILLING_TEAMS view
    CREATE OR REPLACE VIEW OL_JAPFA.IND_BI.BILLING_TEAMS AS
    SELECT 
        BILLING_TYPE,
        SUM(FINAL_QTY) AS TOTAL_FINAL_QTY
...
CREATE OR REPLACE PROCEDURE OL_JAPFA.IND_SAPHANA_DEV_TN.RUN_BATCH_CALLS_JS()
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
try {
    ...
    snowflake.execute({
        sqlText: `CALL OL_JAPFA.IND_SAPHANA_DEV_TN.SP_RUN_ETL_MEDALLION_D();`
    });

    // Wait for 2 seconds
    snowflake.execute({
        sqlText: `CALL SYSTEM$WAIT(2);`
    });

    // Second procedure call
    snowflake.execute({
        sqlText: `CALL OL_JAPFA.IND_SAPHANA_DEV_TN.SP_LOAD_DIM_FACT_TABLES();`
    });

    // Wait for 10 seconds
    snowflake.execute({
        sqlText: `CALL SYSTEM$WAIT(10);`
    });

    // Third procedure call
    snowflake.execute({
        sqlText: `CALL OL_JAPFA.IND_SAPHANA_DEV_TN.CREATE_TEAM_VIEWS();`
    });

    return "All procedures executed Successfully.";
} catch (err) {
    return "Error during procedure execution: " + err.message;
}
$$;
-----------------------------

call OL_JAPFA.IND.RUN_BATCH_CALLS_JS();
...
CREATE OR REPLACE TASK OL_JAPFA.IND_SAPHANA_DEV_TN.SP_RUN_ETL_TASK
  ...
  SCHEDULE = '11 SECONDS'
AS
call OL_JAPFA.IND_SAPHANA_DEV_TN.RUN_BATCH_CALLS_JS();
...
TRUNCATE OL_JAPFA.IND.DIM_BILLING_TYPE;
TRUNCATE OL_JAPFA.IND.DIM_PRODUCT;
TRUNCATE OL_JAPFA.IND.FACT_SALES;
...
SELECT * FROM OL_JAPFA.IND.FACT_SALES order by FINAL_QTY asc;
SELECT count(*) FROM OL_JAPFA.IND.FACT_SALES where  FINAL_QTY IS NULL;
