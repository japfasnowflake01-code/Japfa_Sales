...
ADD FINAL_QTY NUMBER;


CREATE OR REPLACE VIEW OL_JAPFA.IND.BILLING_TEAMS AS
SELECT 
    BILLING_TYPE,
...
CREATE OR REPLACE VIEW OL_JAPFA.IND.MATERIAL_TEAMS AS
SELECT 
    MATERIAL,
    SUM(FINAL_QTY) AS TOTAL_FINAL_QTY
FROM OL_JAPFA.IND.FACT_SALES
...
SELECT * FROM OL_JAPFA.IND.BILLING_TEAMS;
drop view OL_JAPFA.IND.MATERIAL_TEAMS;



select * from OL_JAPFA.IND.MATERIAL_TEAMS;
...
CREATE OR REPLACE PROCEDURE OL_JAPFA.IND.CREATE_TEAM_VIEWS()
RETURNS STRING
LANGUAGE SQL
AS
$$
...
-- CREATE OR REPLACE PROCEDURE OL_JAPFA.IND.RUN_BATCH_CALLS()
-- RETURNS STRING
-- LANGUAGE SQL
-- AS
-- $$
...
--     -- Third call
--     CALL OL_JAPFA.IND.CREATE_TEAM_VIEWS();

--     LET v_result = 'All procedures executed with delays.';
--     RETURN v_result;
-- END;
-- $$;
...
CREATE OR REPLACE PROCEDURE OL_JAPFA.IND.RUN_BATCH_CALLS_JS()
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
try {
    ...
    snowflake.execute({
        sqlText: `CALL OL_JAPFA.IND_UTIL.SP_LOAD_DIM_FACT_TABLES();`
    });

    // Wait for 120 seconds
    snowflake.execute({
        sqlText: `CALL SYSTEM$WAIT(120);`
    });

    // Third procedure call
    snowflake.execute({
        sqlText: `CALL OL_JAPFA.IND.CREATE_TEAM_VIEWS();`
    });

    return "All procedures executed with delays using JavaScript.";
} catch (err) {
